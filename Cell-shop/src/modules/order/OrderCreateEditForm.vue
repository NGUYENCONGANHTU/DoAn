  ...
  setup(props) {
    ...
    const submitForm = () => {
      handleSubmit(async () => {
        try {
          if (model.id) {
            await store.dispatch("order/updateOrder", model);
            router.push("/order").then(() => {
              toast.success(
                `status: ${Constants.OK200}: edit order successfully`
              );
            });
          } else {
            await store.dispatch("order/createOrder", model);
            router.push("/order").then(() => {
              toast.success(
                `status: ${Constants.OK200}: create order successfully`
              );
            });
          }
          props.onClose(false);
