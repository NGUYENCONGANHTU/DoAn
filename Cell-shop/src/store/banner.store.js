...
const bannerStore = {
  ...
  actions: {
    ...
    async deleteBanner({ commit }, id) {
      try {
        commit("isLoading", true);
        const response = await bannerService.deleteBanner(
          `/api/admins/banner/destroy/${id}`
        );
        if (response) {
          commit("deleteCakeBanner", id);
          commit("isLoading", false);
        }
      } catch (error) {
        commit("isLoading", true);
